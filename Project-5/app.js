const UI = (() => {
  ...
  function applyTheme(theme) {
    ...
    if (theme === "light") themeIcon.className = "bi bi-sun";
    else themeIcon.className = "bi bi-moon-stars";
    // additional tweaks for light theme to make cards and text more readable
    if (theme === "light") {
      document
        .querySelectorAll(".card")
        .forEach((el) => (el.style.background = "var(--panel)"));
      document
        .querySelectorAll(".booking-item")
        .forEach(
          (el) =>
            (el.style.background =
              "linear-gradient(180deg, rgba(2,6,23,0.02), rgba(2,6,23,0.01))")
        );
    } else {
      document
        .querySelectorAll(".card")
  ...
  function renderBookings() {
    ...
    bookings.forEach((bk, i) => {
      ...
      el.innerHTML = `<div><strong>${bk.provider.name}</strong><div class="muted small">${bk.date} • ${bk.time}</div></div><div><button data-i='${i}' class='btn btn-ghost small'><i class='bi bi-trash'></i></button></div>`;
  ...
  function renderRecentFor(providerId, dateStr) {
    ...
    list.forEach((bk, idx) => {
      const el = document.createElement("div");
      el.className = "booking-item";
      el.innerHTML = `<div><strong>${bk.time}</strong><div class='muted small'>${bk.provider.name}</div></div><div><button data-slot='${bk.slotId}' class='delete-btn' title='Delete booking'><i class='bi bi-trash'></i></button></div>`;
  ...
  async function syncClock() {
    try {
      ...
      statClock.textContent = new Date(j.utc_datetime)
        .toUTCString()
        .replace("GMT", "UTC");
    } catch (e) {
      statClock.textContent = new Date().toUTCString().replace("GMT", "UTC");
    }
  }
